...
class BootstrapCode extends CrudCode
{
    ...
    public function generateActiveControlGroupFlat($modelClass, $column)
    {
        if ($column->type === 'boolean') {
            return "\$form->checkBoxControlGroup(\$model$modelClass, '{$column->name}')";
        } else {
            if (stripos($column->dbType, 'text') !== false) {
                return "\$form->textAreaControlGroup(\$model".$modelClass.", '{$column->name}', array( 'rows' => 6, 'class' => 'input-block-level', 'placeholder' => \$model".$modelClass."->getAttributeLabel('{$column->name}')))";
            } else {
                if (preg_match('/^(password|pass|passwd|passcode)$/i', $column->name)) {
                    $inputField = 'passwordFieldControlGroup';
                } else {
                ...
                if ($column->type !== 'string' || $column->size === null) {
                    return "\$form->{$inputField}(\$model".$modelClass.", '{$column->name}', array('class' => 'input-xlarge', 'placeholder' => \$model".$modelClass."->getAttributeLabel('{$column->name}')))";
                } else {
                    return "\$form->{$inputField}(\$model".$modelClass.", '{$column->name}', array('maxlength' => $column->size, 'class' => 'input-xlarge', 'placeholder' => \$model".$modelClass."->getAttributeLabel('{$column->name}')))";
                }
            }
        }
    }
