...
class TbApi extends CApplicationComponent
{
    ...
    /**
     * Edited be npbtrac 20140624
     * Registers jQuery and Bootstrap JavaScript.
     * @param string $url the URL to the JavaScript file to register.
     * @param int $position the position of the JavaScript code.
     * @param exclude or not exclude when combining
     */
    public function registerCoreScripts($url = null, $position = CClientScript::POS_END, $excludeFromCombining=false)
    {
        if ($url === null) {
        ...
        /** @var CClientScript $cs */
        // $cs must be NpMinScript
        $cs = Yii::app()->getClientScript();
        $cs->registerCoreScript('jquery');
        $cs->registerScriptFile($url, $position, array(), $excludeFromCombining);
    }
